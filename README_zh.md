...
NewBook **不会发起任何网络请求**。所有功能完全在你本地设备上运行：

| 关注点 | 状态 |
|--------|------|
| 外部字体（Google Fonts） | ❌ 已移除 — 完全使用系统字体 |
| 网站图标加载 | ✅ Chrome 内置 `_favicon` API（本地） |
| 搜索请求 | ✅ 直接在新标签页打开搜索结果 — 不经由任何中间服务 |
| 数据统计 / 遥测 | ❌ 无 |
| 数据存储 | ✅ `chrome.storage.sync`（仅搜索引擎配置） |
| 书签数据 | ✅ 直接通过 `chrome.bookmarks` API 读写 |

[内容安全策略 (CSP)](manifest.json) 明确禁止所有外部连接（`connect-src 'none'`）。
